...
export async function reorderEquipment(orderedCodes: string[], options: { preserveOrderOnly?: boolean; module?: string } = {}): Promise<Equipment[]> {
  return apiFetch<Equipment[]>('/equipment/reorder', {
    method: 'POST',
    body: JSON.stringify({ orderedCodes, ...options })
  });
}
