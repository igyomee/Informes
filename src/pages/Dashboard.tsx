import { AlertTriangle, ClipboardCheck, FileText, Wrench } from 'lucide-react';
...
export function Dashboard() {
  ...
  const metrics = [
    ...
    { label: 'Incidencias crÃ­ticas', value: data.incidenciasCriticas, icon: AlertTriangle, tone: 'text-black bg-brand' },
    ...
    <div className="space-y-6">
      {data.mockMode ? (
        <div className="rounded-lg border border-black bg-yellow-100 p-4 text-sm font-semibold text-black">
          La app estÃ¡ funcionando con datos mock porque no hay credenciales de Google Sheets configuradas.
        </div>
      ) : null}
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <Link className="panel p-5 transition hover:border-brand" to="/revision-centro">
          ...
          <h2 className="mt-3 text-lg font-bold text-ink">Revisión por centro</h2>
        </Link>
