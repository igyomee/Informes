...
export function CenterMaintenance() {
  ...
  async function saveReorder() {
    ...
    try {
      ...
      if (isDoors) {
        const updatedByCode = new Map(updated.map((item) => [item.codigo, item]));
        const orderedUpdated = orderedCodes.map((code) => updatedByCode.get(code)).filter((item): item is Equipment => Boolean(item));
        const rest = updated.filter((item) => !orderedCodes.includes(item.codigo));
        setEquipment([...orderedUpdated, ...rest]);
      } else {
