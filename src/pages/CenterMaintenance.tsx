...
function compareByEquipmentNumber(a: Equipment, b: Equipment): number {
  const left = Number.parseInt(a.numeroEquipo || '', 10);
  const right = Number.parseInt(b.numeroEquipo || '', 10);
...
export function CenterMaintenance() {
  ...
  const centerEquipment = useMemo(
    () => {
      ...
      return isDoors ? rows.sort(compareByEquipmentNumber) : rows.sort(compareByEquipmentCode);
    },
  ...
  async function saveReorder() {
    ...
    setError('');
    try {
      const orderedCodes = visibleEquipment.map((item) => item.codigo);
      const updated = await reorderEquipment(orderedCodes, {
        preserveOrderOnly: isDoors,
        module: module.key
      });
      if (isDoors) {
        setEquipment(updated.sort(compareByEquipmentNumber));
      } else {
        setEquipment(updated);
      }
      setReorderMode(false);
      setReorderCodes([]);
      setMessage(isDoors ? 'Orden de maquinas guardado.' : 'Maquinas reordenadas y numeracion actualizada.');
    } catch (err) {
