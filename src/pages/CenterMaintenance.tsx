...
export function CenterMaintenance() {
  ...
  async function addEquipment() {
    ...
    if (!newMachine.codigo || !newMachine.familia || !newMachine.tagFamilia) {
      setError('Indica código, familia y tag de familia.');
      return;
    }

    const created = await createEquipment({
      codigo: newMachine.codigo,
      centro: center,
      tagCentro: newMachine.codigo.split('-')[0] ?? '',
      ...
      numeroEquipo: newMachine.numeroEquipo,
      ubicacion: newMachine.ubicacion,
      activo: true
    });
    setEquipment((current) => [...current, created]);
    setNewMachine({ codigo: '', nombreEquipo: '', numeroEquipo: '', familia: '', tagFamilia: '', ubicacion: '' });
  ...
  async function saveReviewForMachine(item: Equipment): Promise<void> {
    ...
    await createReview({
      ...
      prioridad: hasIncident ? 'Alta' : 'Media',
    ...
    <div className="space-y-4">
      ...
      <section className="panel p-4">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-3">
          <input className="form-input" placeholder="Código" value={newMachine.codigo} onChange={(event) => setNewMachine((v) => ({ ...v, codigo: event.target.value }))} />
          <input
          ...
          <input
            className="form-input"
            placeholder="Número"
            value={newMachine.numeroEquipo}
            onChange={(event) => setNewMachine((v) => ({ ...v, numeroEquipo: event.target.value }))}
          />
          <select
            className="form-input"
          ...
          <input
            className="form-input"
            placeholder="Tag familia"
            value={newMachine.tagFamilia}
            onChange={(event) => setNewMachine((v) => ({ ...v, tagFamilia: event.target.value }))}
          />
          <input
            className="form-input"
            placeholder="Ubicación"
            value={newMachine.ubicacion}
            onChange={(event) => setNewMachine((v) => ({ ...v, ubicacion: event.target.value }))}
          />
        </div>
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-black">
                    {item.familia} · Nº {item.numeroEquipo || '-'} · {item.ubicacion || 'Sin ubicación'}
                  </p>
                </div>
                ...
                <div className="border-t border-black p-4">
                  <div className="grid gap-3 md:grid-cols-3">
                    <input
                      ...
                      placeholder="Ubicación"
                      onChange={(event) =>
                        setEquipment((current) => current.map((row) => (row.codigo === item.codigo ? { ...row, ubicacion: event.target.value } : row)))
                      }
                    />
                    <input
                      className="form-input"
