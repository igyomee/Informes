...
export function EquipmentSearch() {
  ...
  const [filters, setFilters] = useState<EquipmentFilters>({ sortBy: 'nombreEquipo', sortDir: 'asc' });
  ...
  return (
    <div className="space-y-4">
      <FiltersBar
        ...
        onChange={setFilters}
        onReset={() => setFilters({ sortBy: 'nombreEquipo', sortDir: 'asc' })}
      />

      {error ? <ErrorBanner message={error} /> : null}
      {loading ? <LoadingState /> : equipment.length === 0 ? <EmptyState title="No hay máquinas con esos filtros" /> : null}
      {!loading && equipment.length > 0 ? (
        <EquipmentList equipment={equipment} sortBy={filters.sortBy} sortDir={filters.sortDir} onSort={sortBy} />
      ) : null}
    </div>
  );
}
