...
Ultimo ajuste de numeracion al anadir maquinas:

- Al anadir una maquina nueva, el numero se genera con dos digitos: `01`, `02`, `03`, `04`.
- Si ya existen `AC-PA-PB-01`, `AC-PA-PB-02` y `AC-PA-PB-03`, la siguiente sera `AC-PA-PB-04`.
- Para aplicar este cambio, pega y despliega de nuevo estos tres Apps Script:
  - `google-apps-script/clima/Code.gs`
  - `google-apps-script/electrico/Code.gs`
  - `google-apps-script/puertas/Code.gs`

Ultimo ajuste de nombres al anadir maquinas de clima:

- Al anadir una maquina nueva en Clima, la web genera ahora:
  - `UNIDAD INTERIOR CLIMATIZACIÓN NOMBRE`
  - `UNIDAD EXTERIOR CLIMATIZACIÓN NOMBRE`
- Apps Script tambien lo fuerza al guardar, para evitar que se cree sin la palabra `CLIMATIZACIÓN`.
- Para aplicar el cambio completo:
  - Sube a GitHub `src/pages/CenterMaintenance.tsx`.
  - Pega y despliega de nuevo `google-apps-script/clima/Code.gs`.
  - Si mantienes los tres Apps Script iguales, pega tambien `google-apps-script/electrico/Code.gs` y `google-apps-script/puertas/Code.gs`.

Ultimo ajuste de orden para Puertas:

- En Puertas la web ya no ordena automaticamente por tipo/familia.
- El listado respeta el orden real del Google Sheets, para seguir el plano de los tecnicos.
- Al usar `Reordenar maquinas` en Puertas, se guarda ese orden en la hoja, aunque los tipos esten mezclados.
- La numeracion del codigo se mantiene por tipo/familia: una puerta rapida puede ser `01`, una ignifuga `01` y la siguiente rapida `02`.
- Al anadir una puerta nueva, el numero siguiente se calcula dentro de su mismo tipo/familia.
- Para aplicar el cambio completo:
  - Sube a GitHub `src/pages/CenterMaintenance.tsx`.
  - Pega y despliega de nuevo `google-apps-script/puertas/Code.gs`.

Correccion adicional para Puertas:

- Se ha eliminado la ordenacion automatica por codigo/tipo tambien al importar desde Fichas QR y al sincronizar con el Google Sheets general.
- Apps Script detecta Puertas por el contenido de la hoja (`Tag instalación` = `PA` o codigos con `-PA-`), no solo por el nombre del archivo.
- Con esto Puertas debe poder quedarse en el orden manual que pongas en la hoja o con el boton `Reordenar maquinas`.
- Al reordenar Puertas, el script solo mueve las filas y no cambia los codigos existentes.
- Al anadir una puerta nueva, la numeracion del codigo sigue siendo por tipo/familia.

Correccion final de guardado de orden en Puertas:

- La web envia ahora `preserveOrderOnly: true` y `module: puertas` al guardar el orden.
- Apps Script ya no depende solo de detectar Puertas automaticamente: si recibe esa bandera, guarda exactamente el orden enviado.
- La web conserva en pantalla el orden elegido despues de guardar, incluso si el backend devuelve todas las filas del inventario.
- Archivos necesarios:
  - `src/services/equipmentService.ts`
  - `src/pages/CenterMaintenance.tsx`
  - `google-apps-script/puertas/Code.gs`

Nuevo criterio de orden en Puertas:

- En la web, Puertas se ordena por la columna `Número equipo`.
- El `Código del equipo` mantiene otra numeracion independiente por tipo/familia.
- Ejemplo: una puerta rapida puede tener codigo `...-PR-01`, una ignifuga `...-PI-01` y otra rapida `...-PR-02`, pero el orden visual lo marca `Número equipo` 1, 2, 3.
- Al usar `Reordenar maquinas` en Puertas, se actualiza `Número equipo` siguiendo el orden elegido y no se cambia el codigo.
- Al anadir una puerta nueva, el codigo se calcula por tipo/familia y `Número equipo` se pone al final del orden visual.
- Al modificar el tipo/familia de una puerta existente, puede cambiar el codigo si hace falta, pero se conserva `Número equipo`.
