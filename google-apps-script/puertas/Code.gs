...
function sortSheetByEquipmentCode_(sheetName, codeHeader) {
  ...
  if (values.length <= 2) return;
  if (sheetName === SHEETS.INVENTORY && valuesLookLikePuertas_(values)) return;
...
function syncFichasQrFromCurrentInventory_() {
  ...
  if (sourceSheetName !== 'PUERTAS AUTOMATICAS') rows.sort(compareEquipmentObjects_);
...
function replaceInventoryWithImportedRows_(sourceSheetName) {
  var rows = buildImportedEquipment_(sourceSheetName);
  if (sourceSheetName !== 'PUERTAS AUTOMATICAS') rows.sort(compareEquipmentObjects_);
...
function valuesLookLikePuertas_(values) {
  if (!values || values.length < 2) return false;
  var headers = values[0].map(function (header) { return normalizeForSearch_(header); });
  var tagInstallationIndex = headers.indexOf(normalizeForSearch_('Tag instalación'));
  var installationIndex = headers.indexOf(normalizeForSearch_('Instalación'));
  var codeIndex = headers.indexOf(normalizeForSearch_('Código del equipo'));
  for (var i = 1; i < values.length; i++) {
    var row = values[i];
    var tagInstallation = tagInstallationIndex >= 0 ? normalizeForSearch_(row[tagInstallationIndex]) : '';
    var installation = installationIndex >= 0 ? normalizeForSearch_(row[installationIndex]) : '';
    var code = codeIndex >= 0 ? normalize_(row[codeIndex]).toUpperCase() : '';
    if (tagInstallation === 'PA' || installation.indexOf('PUERTA') !== -1 || code.indexOf('-PA-') !== -1) return true;
  }
  return false;
}
...
function reorderEquipment_(data) {
  ...
  if (keepSheetOrder) {
    var selectedQueueForDoors = selected.slice();
    var doorRows = rows.map(function (item) {
      return orderIndex[normalizeForSearch_(item.codigo)] != null ? selectedQueueForDoors.shift() : item;
    });

    var doorSheet = getSheet_(SHEETS.INVENTORY, true);
    doorSheet.clearContents();
    doorSheet.getRange(1, 1, 1, INVENTORY_HEADERS.length).setValues([INVENTORY_HEADERS]);
    if (doorRows.length) {
      doorSheet.getRange(2, 1, doorRows.length, INVENTORY_HEADERS.length).setValues(doorRows.map(equipmentToRow_));
    }
    clearSheetCache_(SHEETS.INVENTORY);
    syncFichasQrFromCurrentInventory_();
    return doorRows.map(stripInternal_);
  }
