...
function reorderEquipment_(data) {
  ...
  var updatedRows = rows.map(function (item) {
...
function dashboard_() {
  ...
  var centers = {};
  var pendingCenters = {};
  var pendingEquipment = equipment.filter(function (item) {
    return item.activo && !reviewedCodesThisMonth[item.codigo];
  });
  equipment.forEach(function (item) {
    if (normalize_(item.centro)) centers[normalizeForSearch_(item.centro)] = true;
  });
  pendingEquipment.forEach(function (item) {
    if (normalize_(item.centro)) pendingCenters[normalizeForSearch_(item.centro)] = true;
  });
  ...
  return {
    ...
    centrosRevisadosMes: Object.keys(reviewedCentersThisMonth).length,
    equiposPendientes: pendingEquipment.length,
    centrosPendientes: Object.keys(pendingCenters).length,
    mockMode: false
  };
}
