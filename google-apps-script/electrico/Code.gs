...
function createEquipment_(data) {
  data = data || {};
  if (!normalize_(data.codigo)) throw new Error('El código del equipo es obligatorio.');
  ...
  var existing = readObjects_(SHEETS.INVENTORY).map(equipmentFromRow_).find(function (item) {
    return normalizeForSearch_(item.codigo) === normalizeForSearch_(data.codigo);
  });
  if (existing) throw new Error('Ya existe un equipo con código ' + data.codigo + '.');

  var parsed = parseEquipmentCodeLoose_(data.codigo);
  var template = findTemplateForTags_(data.tagInstalacion || parsed.tagInstalacion, data.tagFamilia || parsed.tagFamilia);
  var equipment = {
    codigo: normalize_(data.codigo).toUpperCase(),
    centro: normalize_(data.centro),
    tagCentro: normalize_(data.tagCentro || parsed.tagCentro),
    instalacion: normalize_(data.instalacion),
    tagInstalacion: normalize_(data.tagInstalacion || parsed.tagInstalacion),
    familia: normalize_(data.familia),
    tagFamilia: normalize_(data.tagFamilia || parsed.tagFamilia),
    nombreEquipo: normalize_(data.nombreEquipo),
    numeroEquipo: normalize_(data.numeroEquipo || parsed.numero),
...
function validateReviewAnswers_(items, answers) {
  ...
  items.forEach(function (item) {
    ...
    if (answer && isNegativeAnswer_(response)) {
      ...
      if (item.requiereFotoSiFalla && !normalize_(answer.fotoEvidencia)) {
...
function createReview_(payload) {
  ...
  var details = payload.answers.filter(function (answer) {
    ...
    return {
      ...
      prioridad: answer.prioridad || payload.prioridad || 'Media'
    };
  });
