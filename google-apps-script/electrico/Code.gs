...
function buildImportedEquipment_(sourceSheetName) {
  ...
  for (var i = 1; i < values.length; i++) {
    var codigo = cell_(values, i, 9);
    if (!codigo) continue;

    var tagInstalacion = cell_(values, i, 4);
    var tagFamilia = cell_(values, i, 8);
    rows.push({
      codigo: codigo,
      centro: cell_(values, i, 1),
      tagCentro: cell_(values, i, 2),
      instalacion: cell_(values, i, 3),
      tagInstalacion: tagInstalacion,
      subinstalacion: cell_(values, i, 5),
      tagSubinstalacion: cell_(values, i, 6),
      familia: cell_(values, i, 7),
      tagFamilia: tagFamilia,
      nombreEquipo: cell_(values, i, 10),
