...
function getChecklistForEquipment_(codigo) {
  ...
  var mapRow = mapRows.find(function (row) {
    return normalizeForSearch_(row['Tag instalación']) === normalizeForSearch_(parsed.tagInstalacion) &&
      normalizeForSearch_(row['Tag familia']) === normalizeForSearch_(parsed.tagFamilia);
  });

  var plantillaRevision = equipment.plantillaRevision || (mapRow && mapRow['Plantilla revisión']);
